...
.hero h1{margin:0;font-size:clamp(1.45rem,2.4vw,2.15rem);line-height:1.18;white-space:normal;overflow:visible}.hero p{margin:.55rem 0 0;color:#e4f7fa}
...
with st.sidebar:
    ...
    department_choice = st.selectbox("Department", ["All Departments"] + departments, index=0)
    selected_departments = departments if department_choice == "All Departments" else [department_choice]
    pages = ["Executive Command Center", "Initiative Registry", "Value & Evidence", "Governance Action Center", "Governance & Risk", "Adoption & Maturity", "Incidents & Actions", "Data Quality"]
...
if page == "Executive Command Center":
    st.markdown('<div class="hero"><div class="brand"><div class="brandmark">C</div><div class="brandname">Clinexa AI</div></div><div class="badge">Value &amp; Governance Module</div><h1>Clinical AI Value &amp; Governance Platform</h1><p>Evidence-based portfolio oversight for value, quality, adoption, risk and responsible scale.</p></div>', unsafe_allow_html=True)
    ...
    # Additional executive visuals use the space previously occupied by the action list.
    x1, x2 = st.columns(2)
    with x1:
        lifecycle = f.groupby("Lifecycle", as_index=False).size().rename(columns={"size":"Initiatives"})
        fig = px.bar(lifecycle.sort_values("Initiatives", ascending=False), x="Lifecycle", y="Initiatives", text_auto=True, title="Initiatives by lifecycle stage")
        fig.update_layout(showlegend=False, xaxis_title="", yaxis_title="Initiatives")
        st.plotly_chart(fig, use_container_width=True)
    with x2:
        evidence = f.groupby("Evidence_Strength", as_index=False).size().rename(columns={"size":"Initiatives"})
        order = ["Low", "Medium", "High"]
        evidence["Evidence_Strength"] = pd.Categorical(evidence["Evidence_Strength"], categories=order, ordered=True)
        evidence = evidence.sort_values("Evidence_Strength")
        fig = px.bar(evidence, x="Evidence_Strength", y="Initiatives", color="Evidence_Strength", text_auto=True, title="Evidence maturity")
        fig.update_layout(showlegend=False, xaxis_title="Evidence strength", yaxis_title="Initiatives")
        st.plotly_chart(fig, use_container_width=True)

    high_count = len(f[(f.Risk_Level == "High") & (f.Compliance_Score < 80)])
    weak_count = len(f[f.Evidence_Confidence < 70])
    overdue_count = len(cf[(cf.Status != "Complete") & (cf.Due_Date < pd.Timestamp.today())])
    st.info(f"**Governance attention:** {high_count} high-risk below threshold · {weak_count} need stronger evidence · {overdue_count} overdue actions. Open the **Governance Action Center** page for the initiative-level list.")
...
elif page == "Governance Action Center":
    st.markdown('<div class="hero"><div class="brand"><div class="brandmark">C</div><div class="brandname">Clinexa AI</div></div><div class="badge">Prioritized follow-up</div><h1>Governance Action Center</h1><p>Review exceptions by category and open the affected initiative directly.</p></div>', unsafe_allow_html=True)
    ...
    category = st.selectbox("Action category", [f"High risk ({len(high)})", f"Evidence needed ({len(weak)})", f"Overdue governance ({len(overdue)})"])
    if category.startswith("High risk"):
        if high.empty: st.success("No high-risk initiatives below the compliance threshold.")
        for _, r in high.sort_values(["Compliance_Score","AI_Value_Score"]).iterrows():
        ...
        for _, r in weak.sort_values("Evidence_Confidence").iterrows():
